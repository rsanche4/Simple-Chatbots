...
def get_first_file_name(folder_path):
    try:
        ...
        print("Error occurred while getting the first file name:", e)
        return None

def rename_mp3_to_song(file_path):
    try:
        if file_path.endswith(".mp4") or file_path.endswith(".mp3"):
            new_file_path = os.path.join(os.path.dirname(file_path), "song.mp3")
...
def extract_audio(youtube_video_url, directory_path):
    """
    ...
    try:
        ...
        os.remove(audio_file_path)  # Remove the original audio file after conversion
        return True
    except Exception as e:
...
def youtube_audio_extractor(youtube_video_url, directory_path):
    # Extract audio
    success = extract_audio(youtube_video_url, directory_path)
    if success:
        filename = "sound\\" + get_first_file_name("sound\\")
        rename_mp3_to_song(filename)
        print("The audio file has been saved successfully!")
    else:
        print("Error, the audio file has not been saved!")
